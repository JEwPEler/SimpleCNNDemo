...
checkpoint_file = os.path.join(thisFolder,'inception_resnet_v2_2016_08_30.ckpt')
...
file_pattern = 'traffic_signs_%s_*.tfrecord'

items_to_descriptions = {
    'image': 'A 3-channel RGB coloured traffic_sign image that is either ****, *******, *******, stop, or turen_riget.',
    'label': 'A label that is as such -- 0:********, 1:*******, 2:*******, 3:******, 4:*****'
    }
num_epochs = 1

#onet batch train numgbers
batch_size = 8

#self_make,the num will be train

initial_learning_rate = 0.0002
learning_rate_decay_factor = 0.7
num_epochs_before_decay = 2

class ResnetIv2TrainThread(QThread):

    someStepsOverSin = pyqtSignal(dict)
    finishedTrainSin = pyqtSignal(str)
    runningSin=pyqtSignal()
      
    def __init__(self,parent=None):
        super(ResnetIv2TrainThread,self).__init__()
        self.parent=parent
        self.num_steps=2
    ...
    def run(self):
        ...
        with tf.Graph().as_default() as graph:
            ...
            def train_step(sess, train_op, global_step):
                ...
                self.someStepsOverSin.emit({"step":global_step_count,"loss":total_loss,
            ...
            with sv.managed_session() as sess:
                ...
                self.finishedTrainMessage='Finished trianing!model had saved!Final Loss:%s,Final Accuracy:%s'%(loss,sess.run(accuracy))
                self.finishedTrainSin.emit(self.finishedTrainMessage)
                return 



def get_split(split_name, dataset_dir, file_pattern=file_pattern, file_pattern_for_counting='traffic_signs'):
    '''
...
class Main(QWidget):  
    def __init__(self, parent = None):  
        ...
        self.thread = ResnetIv2TrainThread(self)
        self.thread.setNumSteps(1)
        self.thread.start()
